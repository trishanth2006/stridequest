...
| 02D-03 | Territory domain types & contracts (✅ done). | `features/territory/types.ts`, `features/territory/mappers.ts` | 02D-01 | S | `CellId`, `TerritoryAction` union, domain types + row aliases, pure mappers (`isTerritoryAction`, `to*`); no business logic, no H3; unit-tested. |
| 02D-03A | Grid abstraction module. | `features/territory/services/grid.ts` | 02D-03, A1 confirmed | M | Single module exposing `pathToCells(linestring, resolution)` and `cellToCenter(id)`; H3 is the implementation behind the interface; pure & unit-tested. |
| 02D-04 | Capture service. | `features/territory/services/capture.ts` | 02D-03A | M | Pure function `(linestring) => Set<cellId>`; deterministic; unit-tested with fixture geometries. |
