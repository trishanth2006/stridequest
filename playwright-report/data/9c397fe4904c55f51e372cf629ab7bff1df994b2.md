...
- Name: run\start-workout.spec.ts >> Workout lifecycle — /run >> stop transitions a recording workout to completed
- Location: tests\e2e\run\start-workout.spec.ts:20:7
...
```ts
  ...
  3   | 
  4   | test.describe('Workout lifecycle — /run', () => {
  5   |   test('unauthenticated visit to /run redirects to /login', async ({ page }) => {
  6   |     await page.goto('/run')
  7   |     await expect(page).toHaveURL(/\/login/)
  8   |   })
  9   | 
  10  |   test('start transitions the UI to a recording state', async ({ page }) => {
  11  |     const user = uniqueUser()
